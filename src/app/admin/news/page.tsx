...
import { getNewsList, deleteNews } from '@/lib/firebase/services/news'
...
export default function AdminNewsPage() {
  ...
  async function load() {
    try {
      ...
      setError('โหลดข้อมูลไม่สำเร็จ — ตรวจสอบการเชื่อมต่อ Firebase')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="max-w-5xl mx-auto">
      <AdminPageHeader title="📰 ข่าวสาร" createHref="/admin/news/new" />
      ...
      <NeoCard hover={false} size="sm">
        {loading ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="font-medium mb-1">ยังไม่มีข่าวสาร</p>
            <p className="text-sm text-[var(--color-text-muted)] mb-5">สร้างข่าวแรกของคุณเลย</p>
            <Link href="/admin/news/new">
              <NeoButton variant="primary" size="sm">+ สร้างข่าวสาร</NeoButton>
            </Link>
          </div>
        ) : (
                ...
                {items.map(item => (
                  <tr key={item.id} className="border-b border-[var(--color-border)]/50 hover:bg-[var(--color-bg)]">
                    <td className="py-2.5 px-3">
                      ...
                      <p className="text-xs text-[var(--color-text-muted)] mt-0.5">/{item.slug}</p>
                    </td>
                    ...
                    <td className="py-2.5 px-3 text-right">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <NeoButton variant="danger" size="sm" onClick={() => handleDelete(item.id, item.title)}>
                          ลบ
                        </NeoButton>
                      </div>
                    </td>
