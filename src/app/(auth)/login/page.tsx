...
function LoginForm() {
  ...
  const getErrorMessage = (code: string) => {
    const map: Record<string, string> = {
      ...
      'auth/popup-closed-by-user': 'ปิดหน้าต่าง Google Sign-in ก่อนเสร็จสิ้น',
      'auth/popup-blocked': 'เบราว์เซอร์บล็อก popup กรุณาอนุญาต popup แล้วลองใหม่',
      'auth/unauthorized-domain': 'Domain ไม่ได้รับอนุญาต — กรุณาเพิ่ม localhost ใน Firebase Authorized domains',
      'auth/cancelled-popup-request': 'ปิดหน้าต่าง Google Sign-in ก่อนเสร็จสิ้น',
    }
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const tokenResult = await useAuthStore.getState().user?.getIdTokenResult()
      const userRole = tokenResult?.claims['role'] ?? 'member'
      router.push(userRole === 'admin' || userRole === 'superadmin' ? '/admin' : redirect)
    } catch (err: unknown) {
